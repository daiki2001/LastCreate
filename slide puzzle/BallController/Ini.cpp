...
Ini::Ini(const char* filePath) {
    ...
    if (!file) this->raise(std::string("File not found: ") + filePath);
    ...
    while(getline(file, line)) {
        ...
        if (line[0] == '[') {
            ...
            if (endIndex == std::string::npos) this->raise("Invalid section: " + line);
            section = line.substr(1, endIndex - 1);
            this->content[section] = std::map<std::string, std::string>();
        } else {
            // key’Tõ
            const size_t equalIndex = line.find(" = ");
            if (equalIndex == std::string::npos) this->raise("Invalid key/value: " + line);
            // keyÝ’è
...
std::string Ini::getValue(const char* section, const char* key) {
    if (!this->content.count(section)) this->raise(std::string("Section not found: ") + section);
    if (!this->content[section].count(key)) this->raise(std::string("Key not found: ") + key);
